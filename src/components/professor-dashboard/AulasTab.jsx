...
const RescheduleDialog = ({ appointment, isOpen, onClose, onReschedule }) => {
    ...
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [availableTimes, setAvailableTimes] = useState(ALL_TIMES.filter(time => time >= '07:00' && time <= '23:30')); // Estado inicial filtrado
    ...
    const fetchAvailableSlots = useCallback(async (date) => {
        ...
        const newAvailableTimes = ALL_TIMES.filter(time => {
            ...
            // Verifica se o horário é ativo nas Preferências
            if (!preferredTimes.has(time)) return false; 
            
            // Verifica se o slot inicial já está reservado
            ...
            for (let i = 0; i < slotsPerClass; i++) {
                const requiredSlotTime = format(new Date(startTimeObj.getTime() + i * 15 * 60000), 'HH:mm');
                ...
                // Verifica se os slots subsequentes estão nas preferências ativas (apenas slots de 15 minutos)
                if (i > 0 && !preferredTimes.has(requiredSlotTime)) {
                    return false;
                }
            }
            return true;
